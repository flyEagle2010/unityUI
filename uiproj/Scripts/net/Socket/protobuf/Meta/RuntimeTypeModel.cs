...
namespace ProtoBuf.Meta
{
    ...
    public sealed class RuntimeTypeModel : TypeModel
    {
        ...
        private const byte
           OPTIONSInferTagFromNameDefault = 1,
           OPTIONSIsDefaultModel = 2,
           OPTIONSFrozen = 4,
           OPTIONSAutoAddMissingTypes = 8,
#if FEATCOMPILER && !FX11
           OPTIONSAutoCompile = 16,
#endif
           OPTIONSUseImplicitZeroDefaults = 32,
           OPTIONSAllowParseableTypes = 64,
           OPTIONSAutoAddProtoContractTypesOnly = 128;
        ...
        {
            get { return GetOption(OPTIONSInferTagFromNameDefault); }
            set { SetOption(OPTIONSInferTagFromNameDefault, value); }
        }
        ...
        {
            get { return GetOption(OPTIONSAutoAddProtoContractTypesOnly); }
            set { SetOption(OPTIONSAutoAddProtoContractTypesOnly, value); }
        }
        ...
        {
            get {return GetOption(OPTIONSUseImplicitZeroDefaults);}
            set {
                if (!value && GetOption(OPTIONSIsDefaultModel))
                {
                    throw new InvalidOperationException("UseImplicitZeroDefaults cannot be disabled on the default model");
                }
                SetOption(OPTIONSUseImplicitZeroDefaults, value);
            }
        }
        ...
        {
            get { return GetOption(OPTIONSAllowParseableTypes); }
            set { SetOption(OPTIONSAllowParseableTypes, value); }
        }
        ...
        internal RuntimeTypeModel(bool isDefault)
        {
            ...
            SetOption(OPTIONSIsDefaultModel, isDefault);
#if FEATCOMPILER && !FX11 && !DEBUG
            AutoCompile = true;
#endif
        }
...
//#endif
        ...
        {
            get { return GetOption(OPTIONSAutoCompile); }
            set { SetOption(OPTIONSAutoCompile, value); }
        }
#endif
        /// <summary>
        ...
        {
            get { return GetOption(OPTIONSAutoAddMissingTypes); }
            set {
                if (!value && GetOption(OPTIONSIsDefaultModel))
                {
                ...
                ThrowIfFrozen();
                SetOption(OPTIONSAutoAddMissingTypes, value);
            }
        }
        /// <summary>
        ...
        {
            if (GetOption(OPTIONSFrozen)) throw new InvalidOperationException("The model cannot be changed once frozen");
        }
        /// <summary>
        ...
        public void Freeze()
        {
            if (GetOption(OPTIONSIsDefaultModel)) throw new InvalidOperationException("The default model cannot be frozen");
            SetOption(OPTIONSFrozen, true);
        }
        ...
        public TypeModel Compile(CompilerOptions options)
        {
            ...
            {
                asm.SetAssemblyKeyPair(new StrongNameKeyPair(File.OpenRead(options.KeyFile)));
            }
            else if (!Helpers.IsNullOrEmpty(options.KeyContainer))
            {
                asm.SetAssemblyKeyPair(new StrongNameKeyPair(options.KeyContainer));
            }
            else if (!Helpers.IsNullOrEmpty(options.PublicKey))
            {
                asm.SetAssemblyPublicKey(FromHex(options.PublicKey));
            }
            if(!Helpers.IsNullOrEmpty(options.ImageRuntimeVersion) && options.MetaDataVersion != 0)
            {
                asm.SetImageRuntimeVersion(options.ImageRuntimeVersion, options.MetaDataVersion);
            }
        ...
        private Compiler.CompilerContext WriteSerializeDeserialize(string assemblyName, TypeBuilder type, SerializerPair[] methodPairs, Compiler.CompilerContext.ILVersion ilVersion, ref ILGenerator il)
        {
            ...
            for (int i = 0; i < jumpTable.Length; i++)
            {
                ...
                il.Emit(OpCodes.Ldarg2);
                ctx.CastFromObject(pair.Type.Type);
                il.Emit(OpCodes.Ldarg3);
        ...
        private const int KnownTypesArray = 1, KnownTypesDictionary = 2, KnownTypesHashtable = 3, KnownTypesArrayCutoff = 20;
        private void WriteGetKeyImpl(TypeBuilder type, bool hasInheritance, SerializerPair[] methodPairs, Compiler.CompilerContext.ILVersion ilVersion, string assemblyName, out ILGenerator il, out int knownTypesCategory, out FieldBuilder knownTypes, out Type knownTypesLookupType)
        {
            ...
            if (types.Count <= KnownTypesArrayCutoff)
            {
                knownTypesCategory = KnownTypesArray;
            ...
            else
            {
                ...
                if (knownTypesLookupType == null)
                {
                    ...
                    knownTypesCategory = KnownTypesHashtable;
                }
                else
                {
                    knownTypesCategory = KnownTypesDictionary;
                }
            }
                ...
                case KnownTypesArray:
                    {
                        ...
                        if (hasInheritance)
                        {
                            ...
                            il.Emit(OpCodes.Ldloc0); // not a sub-type; use the original value
                            il.Emit(OpCodes.Ret);
                ...
                case KnownTypesHashtable:
                    {
                        ...
                        {
                            il.Emit(OpCodes.UnboxAny, MapType(typeof(int)));
                        }
#endif
                        il.Emit(OpCodes.Ret);
                        il.MarkLabel(otherwise);
...
,
        ...
        private static MethodBuilder EmitBoxedSerializer(TypeBuilder type, int i, Type valueType, SerializerPair[] methodPairs, TypeModel model, Compiler.CompilerContext.ILVersion ilVersion, string assemblyName)
        {
            ...
            MethodBuilder boxedSerializer = type.DefineMethod("" + i.ToString(), MethodAttributes.Static, CallingConventions.Standard,
