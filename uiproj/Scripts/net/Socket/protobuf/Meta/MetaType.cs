...
namespace ProtoBuf.Meta
{
    ...
    public class MetaType : ISerializerProxy
    {
        ...
        {
            if ((flags & OPTIONSFrozen)!=0) throw new InvalidOperationException("The type cannot be changed once a serializer has been generated for " + type.FullName);
        }
        //internal void Freeze() { flags |= OPTIONSFrozen; }
        ...
        internal void ApplyDefaultBehaviour()
        {
            ...
            foreach (MemberInfo member in foundList)
            {
                ...
                if((property = member as PropertyInfo) != null)
                {
                    ...
                    isField = false;
                    ApplyDefaultBehaviourAddMembers(model, family, isEnum, partialMembers, dataMemberOffset, inferTagByName, implicitMode, members, member, ref forced, isPublic, isField, ref effectiveType);
                } else if ((field = member as FieldInfo) != null)
                {
                    ...
                    { // only care about static things on enums; WinRT has a value instance field!
                        continue;
                    }
                    ApplyDefaultBehaviourAddMembers(model, family, isEnum, partialMembers, dataMemberOffset, inferTagByName, implicitMode, members, member, ref forced, isPublic, isField, ref effectiveType);
        ...
        private static void ApplyDefaultBehaviourAddMembers(TypeModel model, AttributeFamily family, bool isEnum, BasicList partialMembers, int dataMemberOffset, bool inferTagByName, ImplicitFields implicitMode, BasicList members, MemberInfo member, ref bool forced, bool isPublic, bool isField, ref Type effectiveType)
        {
            switch (implicitMode)
            {
...
#if !FEATIKVM // IKVM can't access HasValue, but conveniently, Value will only be returned if set via ctor or property
            ...
            { // key-value-pair etc
                ...
                if(ResolveTupleConstructor(type, out mapping) != null)
                {
                    ...
                    for(int i = 0 ; i < mapping.Length ; i++)
                    {
                        ...
                        NewLine(builder, indent + 1).Append("optional ").Append(model.GetSchemaTypeName(effectiveType, DataFormat.Default, false, false, ref requiresBclImport).Replace('.',''))
