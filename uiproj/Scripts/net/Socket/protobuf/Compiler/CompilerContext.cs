...
namespace ProtoBuf.Compiler
{
    ...
    internal sealed class CompilerContext
    {
...
#else
                        il.Emit(OpCodes.UnboxAny, type);
#if DEBUGCOMPILE
                        Helpers.DebugWriteLine(OpCodes.UnboxAny + ": " + type);
#endif
                        break;
#endif
                }
...
#if PLATNOINTERLOCKED
            uniqueIdentifier = ++next;
#else
            uniqueIdentifier = Interlocked.Increment(ref next);
#endif
            method = new DynamicMethod("proto" + uniqueIdentifier.ToString(), returnType, paramTypes, associatedType.IsInterface ? typeof(object) : associatedType, true);
        ...
        {
            il.Emit(OpCodes.LdcR4, value);
#if DEBUGCOMPILE
            Helpers.DebugWriteLine(OpCodes.LdcR4 + ": " + value);
#endif
        }
        public void LoadValue(double value)
        {
            il.Emit(OpCodes.LdcR8, value);
#if DEBUGCOMPILE
            Helpers.DebugWriteLine(OpCodes.LdcR8 + ": " + value);
#endif
        }
        public void LoadValue(long value)
        {
            il.Emit(OpCodes.LdcI8, value);
#if DEBUGCOMPILE
            Helpers.DebugWriteLine(OpCodes.LdcI8 + ": " + value);
#endif
        }
        public void LoadValue(int value)
        {
            switch (value)
            {
                case 0: Emit(OpCodes.LdcI40); break;
                case 1: Emit(OpCodes.LdcI41); break;
                case 2: Emit(OpCodes.LdcI42); break;
                case 3: Emit(OpCodes.LdcI43); break;
                case 4: Emit(OpCodes.LdcI44); break;
                case 5: Emit(OpCodes.LdcI45); break;
                case 6: Emit(OpCodes.LdcI46); break;
                case 7: Emit(OpCodes.LdcI47); break;
                case 8: Emit(OpCodes.LdcI48); break;
                case -1: Emit(OpCodes.LdcI4M1); break;
                default:
                    if (value >= -128 && value <= 127)
                    {
                        il.Emit(OpCodes.LdcI4S, (sbyte)value);
#if DEBUGCOMPILE
                        Helpers.DebugWriteLine(OpCodes.LdcI4S + ": " + value);
#endif
                    }
                    else
                    {
                        il.Emit(OpCodes.LdcI4, value);
#if DEBUGCOMPILE
                        Helpers.DebugWriteLine(OpCodes.LdcI4 + ": " + value);
#endif
                    }
                    break;
        ...
        {
            Emit(isStatic ? OpCodes.Ldarg1 : OpCodes.Ldarg2);
        }
        public void StoreValue(Local local)
        {
            if (local == this.InputValue)
            {
                ...
                il.Emit(OpCodes.StargS, b);
#if DEBUGCOMPILE
                Helpers.DebugWriteLine(OpCodes.StargS + ": $" + b);
#endif                
            }
            else
            {
                ...
                {
                    case 0: Emit(OpCodes.Stloc0); break;
                    case 1: Emit(OpCodes.Stloc1); break;
                    case 2: Emit(OpCodes.Stloc2); break;
                    case 3: Emit(OpCodes.Stloc3); break;
                    default:
#endif
                        OpCode code = UseShortForm(local) ? OpCodes.StlocS : OpCodes.Stloc;
        ...
        public void LoadValue(Local local)
        {
            ...
            {
                Emit(isStatic ? OpCodes.Ldarg0 : OpCodes.Ldarg1);
            }
            else
            {
#if !FX11
                switch (local.Value.LocalIndex)
                {
                    case 0: Emit(OpCodes.Ldloc0); break;
                    case 1: Emit(OpCodes.Ldloc1); break;
                    case 2: Emit(OpCodes.Ldloc2); break;
                    case 3: Emit(OpCodes.Ldloc3); break;
                    default:
#endif             
                        OpCode code = UseShortForm(local) ? OpCodes.LdlocS :  OpCodes.Ldloc;
                        il.Emit(code, local.Value);
#if DEBUGCOMPILE
                        Helpers.DebugWriteLine(code + ": $" + local.Value);
#endif
...
#if FEATIKVM
            foreach (CustomAttributeData attrib in assembly.GetCustomAttributes(attributeType, false))
            {
        ...
        internal static void LoadValue(ILGenerator il, int value)
        {
            switch (value)
            {
                case 0: il.Emit(OpCodes.LdcI40); break;
                case 1: il.Emit(OpCodes.LdcI41); break;
                case 2: il.Emit(OpCodes.LdcI42); break;
                case 3: il.Emit(OpCodes.LdcI43); break;
                case 4: il.Emit(OpCodes.LdcI44); break;
                case 5: il.Emit(OpCodes.LdcI45); break;
                case 6: il.Emit(OpCodes.LdcI46); break;
                case 7: il.Emit(OpCodes.LdcI47); break;
                case 8: il.Emit(OpCodes.LdcI48); break;
                case -1: il.Emit(OpCodes.LdcI4M1); break;
                default: il.Emit(OpCodes.LdcI4, value); break;
            }
        }
        ...
        internal void LoadAddress(Local local, Type type)
        {
            if (type.IsValueType)
            {
                ...
                if (local == this.InputValue)
                {
                    il.Emit(OpCodes.LdargaS, (isStatic ? (byte)0 : (byte)1));
#if DEBUGCOMPILE
                    Helpers.DebugWriteLine(OpCodes.LdargaS + ": $" + (isStatic ? 0 : 1));
#endif
                }
                else
                {
                    OpCode code = UseShortForm(local) ? OpCodes.LdlocaS : OpCodes.Ldloca;
                    il.Emit(code, local.Value);
#if DEBUGCOMPILE
                    Helpers.DebugWriteLine(code + ": $" + local.Value);
#endif
        ...
        {
            OpCode code = @short ? OpCodes.BrS : OpCodes.Br;
        ...
        {
            OpCode code = @short ? OpCodes.BrfalseS :  OpCodes.Brfalse;
            il.Emit(code, label.Value);
#if DEBUGCOMPILE
            Helpers.DebugWriteLine(code + ": " + label.Index);
#endif
        ...
        {
            OpCode code = @short ? OpCodes.BrtrueS : OpCodes.Brtrue;
            il.Emit(code, label.Value);
#if DEBUGCOMPILE
            Helpers.DebugWriteLine(code + ": " + label.Index);
#endif
        ...
        {
            OpCode code = @short ? OpCodes.BeqS : OpCodes.Beq;
            il.Emit(code, label.Value);
#if DEBUGCOMPILE
            Helpers.DebugWriteLine(code + ": " + label.Index);
#endif
        ...
        {
            OpCode code = @short ? OpCodes.BgtS : OpCodes.Bgt;
            il.Emit(code, label.Value);
#if DEBUGCOMPILE
            Helpers.DebugWriteLine(code + ": " + label.Index);
#endif
        ...
        {
            OpCode code = @short ? OpCodes.BltS : OpCodes.Blt;
            il.Emit(code, label.Value);
#if DEBUGCOMPILE
            Helpers.DebugWriteLine(code + ": " + label.Index);
#endif
        ...
        {
            OpCode code = @short ? OpCodes.LeaveS : OpCodes.Leave;
            il.Emit(code, label.Value);
#if DEBUGCOMPILE
            Helpers.DebugWriteLine(code + ": " + label.Index);
#endif
        ...
        internal void LoadArrayValue(Local arr, Local i)
        {
            ...
            switch(Helpers.GetTypeCode(type)) {
                case ProtoTypeCode.SByte: Emit(OpCodes.LdelemI1); break;
                case ProtoTypeCode.Int16: Emit(OpCodes.LdelemI2); break;
                case ProtoTypeCode.Int32: Emit(OpCodes.LdelemI4); break;
                case ProtoTypeCode.Int64: Emit(OpCodes.LdelemI8); break;

                case ProtoTypeCode.Byte: Emit(OpCodes.LdelemU1); break;
                case ProtoTypeCode.UInt16: Emit(OpCodes.LdelemU2); break;
                case ProtoTypeCode.UInt32: Emit(OpCodes.LdelemU4); break;
                case ProtoTypeCode.UInt64: Emit(OpCodes.LdelemI8); break; // odd, but this is what C# does...

                case ProtoTypeCode.Single: Emit(OpCodes.LdelemR4); break;
                case ProtoTypeCode.Double: Emit(OpCodes.LdelemR8); break;
                default:
                    if (type.IsValueType)
                    {
        ...
        internal void ConvertToInt32(ProtoTypeCode typeCode, bool uint32Overflow)
        {
            switch (typeCode)
            {
                ...
                case ProtoTypeCode.Int64:
                    Emit(OpCodes.ConvOvfI4);
                    break;
                case ProtoTypeCode.UInt32:
                    Emit(uint32Overflow ? OpCodes.ConvOvfI4Un : OpCodes.ConvOvfI4);
                    break;
                case ProtoTypeCode.UInt64:
                    Emit(OpCodes.ConvOvfI4Un);
                    break;
                default:
        ...
        internal void ConvertFromInt32(ProtoTypeCode typeCode, bool uint32Overflow)
        {
            switch (typeCode)
            {
                case ProtoTypeCode.SByte: Emit(OpCodes.ConvOvfI1); break;
                case ProtoTypeCode.Byte: Emit(OpCodes.ConvOvfU1); break;
                case ProtoTypeCode.Int16: Emit(OpCodes.ConvOvfI2); break;
                case ProtoTypeCode.UInt16: Emit(OpCodes.ConvOvfU2); break;
                case ProtoTypeCode.Int32: break;
                case ProtoTypeCode.UInt32: Emit(uint32Overflow ? OpCodes.ConvOvfU4 : OpCodes.ConvU4); break;
                case ProtoTypeCode.Int64: Emit(OpCodes.ConvI8); break;
                case ProtoTypeCode.UInt64: Emit(OpCodes.ConvU8); break;
        ...
        //{
        //    Emit(value ? OpCodes.LdcI41 : OpCodes.LdcI40);
        //}
