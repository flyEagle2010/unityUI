...
#if PLATBINARYFORMATTER || (SILVERLIGHT && NET40)
...
#if PLATBINARYFORMATTER
                    else if (parameterType == ctx.MapType(typeof(System.Runtime.Serialization.StreamingContext)))
                    {
                        ctx.LoadSerializationContext();
                        MethodInfo op = ctx.MapType(typeof(SerializationContext)).GetMethod("opImplicit", new Type[] { ctx.MapType(typeof(SerializationContext)) });
                        if (op != null)
